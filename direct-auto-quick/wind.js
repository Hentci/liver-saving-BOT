...
function quest() {
    ...
    const func = () => {
        ...
        if(checkQueBtn()) {
            ...
            for(var i = 1;i <= 10;i++){
                var summonNameElement = document.querySelector("#cnt-quest > div.prt-supporter-list.prt-module > div.prt-supporter-attribute.type4.selected > div:nth-child("+i.toString()+") > div.prt-supporter-info > div.prt-supporter-detail > div.prt-supporter-summon.js-prt-supporter-summon > span.js-summon-name");
                // 確認元素存在並印出 summon-name 的內容
                if (summonNameElement) {
                    var summonName = summonNameElement.textContent;
                    // console.log(summonName);
                    if(summonName === "ゼピュロス"){
                        $("#cnt-quest > div.prt-supporter-list.prt-module > div:nth-child(7) > div:nth-child("+i.toString()+") > div.prt-button-cover").trigger("tap");
