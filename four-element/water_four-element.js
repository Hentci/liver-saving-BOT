...
function result() {
    ...
    const checkBOSSexistBtn = () => $("#pop > div > div.prt-popup-footer > div.btn-usual-next.on").length > 0;
    const func = () => {
        if (checkBOSSexistBtn()){
            $("#pop > div > div.prt-popup-footer > div.btn-usual-next.on").trigger("tap");
        }
        if (checkPopBtn()) {
            popOK();
