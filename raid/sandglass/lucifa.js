...
function select_quest() {
    ...
    const findQuest = (questCnt) => {
        ...
        for(var idx = questCnt - 1;idx >= 1;idx--){
            ...
            if(parseInt(gauge_3) >= 30 && parseInt(num_player) <= 10){
                return [idx, element];
            }
...
function lightSummon(){
    ...
    for(var i = 1;i <= 15;i++){
        ...
        var summonLevelElement = document.querySelector("#cnt-quest > div.prt-supporter-list.prt-module > div.prt-supporter-attribute.type5.selected > div:nth-child("+i.toString()+") > div.prt-supporter-info > div.prt-supporter-detail > div.prt-supporter-summon.js-prt-supporter-summon > span.txt-summon-level");
        // 確認元素存在並印出 summon-name 的內容
        if (summonNameElement) {
            var summonName = summonNameElement.textContent;
            // console.log(summonName);
            if(summonName === "ルシフェル" && summonLevelElement.textContent === "Lv 250"){
